...
Next step we are going to define the node that will make the link between our raw dataset to the target datasets.
...
Here's the whole code of our pipeline node, the function array_to_img is just used to convert a numpy.ndarray to pillow.Image.
...
The other specific part is when you retrieve an item from the Dict, in fact you retrieve the *load()* method bound to the underlying DICOMDataset object.
...
In a nutshell we are going to read 
